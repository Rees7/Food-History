...
public class CalendarDialog extends Dialog implements View.OnClickListener {
    ...
    int selectPos = -1;   //onclick year position
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // add horizontal view to ScrollView
        addViewToLayout();
        initGridView();
    ...
    private void addViewToLayout() {
        hsvViewList = new ArrayList<>();   //manage TextView
        yearList = DBManager.getYearListFromAccounttb(); //get nums of years
        //no record = add this year
        if (yearList.size() == 0) {
            int year = Calendar.getInstance().get(Calendar.YEAR);
            yearList.add(year);
        }

        //nums of year = nums of adding view into ScrollView
        ...
        if (selectPos == -1) {
            selectPos = hsvViewList.size()-1;     //select most recent year
        }
        changeTvbg(selectPos);    //last = onclick
        setHSVClickListener();    //every listener view
    }
    /** add onclick for TextView in horizontal view*/
